...
To learn how to add and edit events, see [Event Sheet View](https://www.construct.net/make-games/manuals/construct-3/interface/event-sheet-view "Make your own games").
...
Events work by **filtering** specific [instances](https://www.construct.net/make-games/manuals/construct-3/project-primitives/objects/instances "Make your own games") that meet some [conditions](https://www.construct.net/make-games/manuals/construct-3/project-primitives/events/conditions "Make your own games"). The [actions](https://www.construct.net/make-games/manuals/construct-3/project-primitives/events/actions "Make your own games") then run for _those instances only_. For example, consider the following event:

[![A simple collision event](https://construct-static.com/images/v1740/r/uploads/articleuploadobject/0/images/934/howeventswork1_v650.png)](https://construct-static.com/images/v1740/uploads/articleuploadobject/0/images/934/howeventswork1.png "Open original 1280x156px in new window (22.07 KB)")

In this example, when a _Bullet_ collides with a _Monster_ the event condition is met. The specific instances of _Bullet_ and _Monster_ that collided in the game are "picked" by the event. Actions only run on the "picked" instances. If there are other instances of _Bullet_ and _Monster_ in the layout, they won't be affected by the _Subtract 1 from health_ and _Destroy_ actions. It would be very difficult to make good games if every bullet hurt every monster!
...
[![An event with two conditions](https://construct-static.com/images/v1740/r/uploads/articleuploadobject/0/images/935/howeventswork2_v650.png)](https://construct-static.com/images/v1740/uploads/articleuploadobject/0/images/935/howeventswork2.png "Open original 1204x105px in new window (20.63 KB)")

This event runs like this:

1. First all _Monsters_ that are on-screen are picked.
...
[![An event with actions on an object not used in conditions](https://construct-static.com/images/v1740/r/uploads/articleuploadobject/0/images/936/howeventswork3_v440.png)](https://construct-static.com/images/v1740/uploads/articleuploadobject/0/images/936/howeventswork3.png "Open original 881x106px in new window (20.73 KB)")

If the user presses Spacebar and the Player's _PowerupEnabled_ flag is set, the action does _Monster: Destroy_. Note that there aren't any conditions that filter or pick Monsters in this event. In this case, _all_ Monster instances are destroyed. In other words, if an event doesn't reference an object in its conditions, actions apply to all the instances of that object.
...
However, **triggers** are an exception. See the green arrow to the left of _Keyboard: On Space pressed_ from the previous example:

[![An event with a triggered condition](https://construct-static.com/images/v1740/r/uploads/articleuploadobject/0/images/936/howeventswork3_v440.png)](https://construct-static.com/images/v1740/uploads/articleuploadobject/0/images/936/howeventswork3.png "Open original 881x106px in new window (20.73 KB)")
...
[![An OR block event in Construct](https://construct-static.com/images/v1740/r/uploads/articleuploadobject/0/images/937/or-block_v650.png)](https://construct-static.com/images/v1740/uploads/articleuploadobject/0/images/937/or-block.png "Open original 1257x153px in new window (27.65 KB)")
...
You can combine the block types by using [sub-events](https://www.construct.net/make-games/manuals/construct-3/project-primitives/events/sub-events "Make your own games"). This allows you to build up more advanced logic, such as an 'Or' block followed by an 'And' block.
...
[![A looping event](https://construct-static.com/images/v1740/r/uploads/articleuploadobject/0/images/938/repeat-event_v600.png)](https://construct-static.com/images/v1740/uploads/articleuploadobject/0/images/938/repeat-event.png "Open original 1164x146px in new window (26.32 KB)")

This means when the layout starts, the _Create object_ action repeats 10 times. The end result is 10 monsters are created at random positions in the layout on startup.

There can also be more conditions following the _Repeat_ condition. These are tested on each of the repeats as well, and must be true for the actions to run. There can even be more than one loop in an event, but this is rare.

## Families and containers

Note families Paid plans only pick their instances entirely separately from any of the object types in the family. For more information, see the section _Picking families in events_ in the manual entry on [Families](https://www.construct.net/make-games/manuals/construct-3/project-primitives/objects/families "Make your own games").

Containers are an advanced feature that can also make groups of instances always be picked together. For more information see the manual entry on [Containers](https://www.construct.net/make-games/manuals/construct-3/project-primitives/objects/containers "Make your own games").
